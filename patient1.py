...
def init_patient():
    ...
    print(f"Patient 1 ready with public key: {public_key}")
...
def send_auth_request():
    ...
    print(f"Patient 1 sent auth request: TS={ts_i}, RN={rn_i}, Encrypted Key={encrypted_key}")
...
def receive_auth_response(response):
    ...
    if abs(ts_gwn_star - ts_gwn) > DELTA_TS:
        print("Patient 1 discarded response: Timestamp too old")
        return
    k_d1_gwn_dec = elgamal_decrypt(private_key, encrypted_key, P)
    print(f"Patient 1 received response: TS={ts_gwn}, RN={rn_gwn}, Decrypted Session Key={k_d1_gwn_dec}")
    session_key = hash_data_timed(f"{k_d1_gwn_dec}{ts_i}{ts_gwn}{rn_i}{rn_gwn}{ID_D1}{ID_GWN}")
    print(f"Patient 1 computed session key: {session_key[:10]}...")
    ...
    while True:
        try:
            ...
            if message[0] == "DISCARD":
                print(f"Patient 1 discarded message: {message[1]}")
                break
            opcode = message[0]
            if opcode == 20:
